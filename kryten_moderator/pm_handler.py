...
Formatting
----------
CyTube PM messages support basic Markdown-style formatting:
    *text*    bold
    __text__  italic
"""
...
def b(text: str) -> str:
    """Return *text* wrapped in CyTube markdown bold."""
    return f"*{text}*"
...
class PMCommandHandler:
    ...
    async def _cmd_help(self, event: ChatMessageEvent, args_str: str) -> None:  # noqa: ARG002
        ...
        lines = [
            b("Command Reference"),
            "Ban user (kicked on join)",
            f"{b('ban')} <user> [reason]",
            "Remove ban",
            f"{b('unban')} <user>",
            "Shadow-mute user (they are unaware)",
            f"{b('smute')} <user> [reason]",
            "Remove shadow mute",
            f"{b('unsmute')} <user>",
            "Mute user",
            f"{b('mute')} <user> [reason]",
            "Remove mute",
            f"{b('unmute')} <user>",
            "List moderated users",
            f"{b('list')} [ban|smute|mute]",
        ...
        if rank >= RANK_ADMIN:
            lines += [
                "List banned patterns",
                f"{b('pattern list')}",
                "Add a banned pattern",
                f"{b('pattern add')} <pattern> [regex] [action] [desc]",
                "Remove a banned pattern",
                f"{b('pattern remove')} <index>",
            ]
        for line in lines:
            await self._reply(event, line)
